...
def receive_server_message(client_t, dm, mode):
    while True:
        try:
            ...
            print(recv_message)
            if mode == "Follower":
                follower = Follower(rancode.decode(recv_message.strip()))
                recv_message = follower.getDecodedCommand()

            if recv_message:
                print("Received: " + recv_message)
                sys.stdout.flush()
            else:
                continue
    ...
    while True:
        try:
            ...
            if mode == "Leader":
                ...
                message = rancode.encode(message)
